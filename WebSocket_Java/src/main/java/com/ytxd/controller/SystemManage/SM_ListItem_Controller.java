 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-列表字段集接口", tags = "系统管理-列表字段集接口")
@Controller("SM_ListItem_Controller")
@RequestMapping(value = "/SystemManage/SM_ListItem")
public class SM_ListItem_Controller extends BaseController{
	@Resource
	private CommonService service;
	@Resource
	private SM_ListCollect_Service serviceLC;

	@ApiOperation(value = "接口说明：列表字段集录入页面接口"
			, notes = "接口说明：列表字段集录入页面接口。<br>"
			+ "使用位置：配置管理-列表字段集-录入<br>"
			+ "逻辑说明：获取列表字段集录入页面搜索条件和列表信息等页面元素")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		if(StringUtils.isNotBlank(request.getParameter("listname"))) {
			obj.put("listname", request.getParameter("listname"));
		}
		obj.put("searchSchema", service.getSearchJson("SM_ListItem_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ListItem_MyList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_ListItem"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ListItem_MyList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ListItem_MyList");
		obj.put("importname", "SM_ListItem_Import");
		obj.put("idfieldname", "id");
		obj.put("sortname", "visible desc,insertindex");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：列表字段集查看页面接口"
			, notes = "接口说明：列表字段集查看页面接口。<br>"
			+ "使用位置：配置管理-列表字段集-查看<br>"
			+ "逻辑说明：获取列表字段集查看页面搜索条件和列表信息等页面元素")
	...
	public Map<String, Object> ViewList_PC(HttpServletRequest request, Model model) throws Exception {
		...
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ListItem_ViewList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ListItem_MyList");
		obj.put("idfieldname", "id");
		obj.put("sortname", "visible desc,insertindex");
	...
	@ApiOperation(value = "接口说明：列表字段集修改页面接口"
			, notes = "接口说明：列表字段集修改页面接口。<br>"
			+ "使用位置：配置管理-列表字段集-修改<br>"
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "listname", value = "listname", required = true, dataType = "String",paramType="query"),
	})
	@PostMapping("/GetList")
	@ResponseBody
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_ListItem_MyList";
		if(request.getParameter("listview") != null) {
			listview = request.getParameter("listview");
		}
		//默认过滤条件
		if(StringUtils.isNotBlank(request.getParameter("listname"))){
			mySqlData.setFieldWhere("listname", request.getParameter("listname"), "=");
		}
	...
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		//保存默认值
		if(StringUtils.isNotBlank(request.getParameter("listname"))){
			mySqlData.setFieldValue("listname", request.getParameter("listname"));
		} else {
			return R.error("listname值不能为空，请修改。");
		}
		//启用缓存
		...
		if(result.getSuccess()) {
			//成功后更新主表ListHtml的值
			serviceLC.UpdateListHtml(request, request.getParameter("listname"));
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		//成功后更新主表ListHtml的值，在删除前先得到一条记录，方便后面得到listname的值
		HashMap<String, Object> mapItem = service.getMap("SM_ListItem", id);
		...
		if (service.delete(mySqlData) > 0) {
			// 成功后更新主表ListHtml的值
			serviceLC.UpdateListHtml(request, DataUtils.getMapKeyValue(mapItem, "listname"));
			return R.ok();
		} else {
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入
		String importname = "SM_ListItem_Import";
