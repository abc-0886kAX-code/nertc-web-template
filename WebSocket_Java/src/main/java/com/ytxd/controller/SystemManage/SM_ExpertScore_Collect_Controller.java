 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-专家打分表配置主表接口", tags = "系统管理-专家打分表配置主表接口")
@Controller("SM_ExpertScore_Collect_Controller")
@RequestMapping(value = "/SystemManage/SM_ExpertScore_Collect")
public class SM_ExpertScore_Collect_Controller extends BaseController{
	@Resource
	private CommonService service;

	@ApiOperation(value = "接口说明：MyList_PC专家打分表配置主表录入列表接口"
			, notes = "接口说明：MyList_PC专家打分表配置主表录入列表接口")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		obj.put("searchSchema", service.getSearchJson("SM_ExpertScore_Collect_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ExpertScore_Collect_MyList"));
		...
		listparam.put("listview", "SM_ExpertScore_Collect_MyList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ExpertScore_Collect_MyList");
		obj.put("importname", "SM_ExpertScore_Collect_Import");
		obj.put("idfieldname", "id");
		obj.put("sortname", "orderid,id");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：ViewList_PC专家打分表配置主表查看列表接口"
			, notes = "接口说明：ViewList_PC专家打分表配置主表查看列表接口")
	@GetMapping("/ViewList_PC")
	@ResponseBody
	public Map<String, Object> ViewList_PC(HttpServletRequest request) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		obj.put("searchSchema", service.getSearchJson("SM_ExpertScore_Collect_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_ExpertScore_Collect_ViewList"));
		SysUser sysuser = DataUtils.getSysUser(request);
		List<String> powerList = new ArrayList<String>();
		powerList.add("export");
		obj.put("powerall", powerList);
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExpertScore_Collect_ViewList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ExpertScore_Collect_MyList");
		obj.put("idfieldname", "id");
		obj.put("sortname", "orderid,id");
	...
	@ApiOperation(value = "接口说明：Add_PC专家打分表配置主表新增页面接口"
			, notes = "接口说明：Add_PC专家打分表配置主表新增页面接口")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_ExpertScore_Collect";
		String viewname = "add";
	...
	@ApiOperation(value = "接口说明：Edit_PC专家打分表配置主表修改页面接口"
			, notes = "接口说明：Edit_PC专家打分表配置主表修改页面接口")
	@ApiImplicitParams({
	...
	@ApiOperation(value = "接口说明：View_PC专家打分表配置主表查看页面接口"
			, notes = "接口说明：View_PC专家打分表配置主表查看页面接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识,必填", required = true, dataType = "String",paramType="query"),
	})
	@GetMapping("/View_PC")
	...
	@ApiOperation(value = "接口说明：GetList专家打分表配置主表获取数据列表接口"
			, notes = "接口说明：GetList专家打分表配置主表获取数据列表接口")
	@ApiImplicitParams({
	...
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_ExpertScore_Collect_MyList";
		if(request.getParameter("listview") != null) {
			listview = request.getParameter("listview");
		}
		mySqlData.setSelectField("ExpertScoreFlag","case when scoresum = 100 then '可用' else '未满100分不可用' end ExpertScoreFlag");
		mySqlData.setJoinSql("SM_ExpertScore_item","left join (select scoreid,sum(scoresum)scoresum from SM_ExpertScore_item group by scoreid) " +
				"SM_ExpertScore_item on SM_ExpertScore_item.scoreid = SM_ExpertScore_Collect.id");
		//默认过滤条件
	...
	@ApiOperation(value = "接口说明：GetListById专家打分表配置主表通过id获取单条数据接口"
			, notes = "接口说明：GetListById专家打分表配置主表通过id获取单条数据接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识,必填", required = true, dataType = "String",paramType="query"),
	})
	...
	@ApiOperation(value = "接口说明：Save专家打分表配置主表保存数据接口"
			, notes = "接口说明：Save专家打分表配置主表保存数据接口")
	@PostMapping("/Save")
	@ResponseBody
	@Transactional
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		MySqlData mySqlData = new MySqlData();
		//数据库表名
		mySqlData.setTableName("SM_ExpertScore_Collect");
	...
	@ApiOperation(value = "接口说明：Delete专家打分表配置主表删除数据接口"
			, notes = "接口说明：Delete专家打分表配置主表删除数据接口。")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识,必填", required = true, dataType = "String",paramType="query"),
	})
	@PostMapping("/Delete")
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		//权限校验
		//执行删除操作
		Integer iResult = service.delete("SM_ExpertScore_Collect", id);
		//删除成功后可以删除关联子表数据
		if(iResult > 0) {
	...
	@ApiOperation(value = "接口说明：ExcelExport专家打分表配置主表Excel导出接口"
			, notes = "接口说明：ExcelExport专家打分表配置主表Excel导出接口")
	@ApiImplicitParams({
	...
	@ApiOperation(value = "接口说明：ExcelImport专家打分表配置主表Excel导入接口"
			, notes = "接口说明：ExcelImport专家打分表配置主表Excel导入接口")
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入
		String importname = "SM_ExpertScore_Collect_Import";
