 ...
 *  控制层(前后台交互)
 */
@Controller("SM_WordCollect_Controller")
@RequestMapping(value = "/SystemManage/SM_WordCollect")
@Api(value = "Word导出总表", tags = "Word导出总表接口")
public class SM_WordCollect_Controller extends BaseController{
	@Resource
	private CommonService service;

	@ApiOperation(value = "接口说明：Word导出总表录入页面接口"
			, notes = "接口说明：Word导出总表录入页面接口。<br>"
			+ "使用位置：配置管理-Word导出总表-录入<br>"
			+ "逻辑说明：获取Word导出总表录入页面搜索条件和列表信息等页面元素")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		obj.put("searchSchema", service.getSearchJson("SM_WordCollect_MyList"));
		obj.put("columnSchema", service.getListJson("SM_WordCollect_MyList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_WordCollect"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_WordCollect_MyList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_WordCollect_MyList");
		obj.put("importname", "SM_WordCollect_Import");
		obj.put("idfieldname", "id");
		obj.put("sortname", "WordName");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：Word导出总表查看页面接口"
			, notes = "接口说明：Word导出总表查看页面接口。<br>"
			+ "使用位置：配置管理-Word导出总表-查看<br>"
			+ "逻辑说明：获取Word导出总表查看页面搜索条件和列表信息等页面元素")
	@GetMapping("/ViewList_PC")
	@ResponseBody
	public Map<String, Object> ViewList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		obj.put("searchSchema", service.getSearchJson("SM_WordCollect_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_WordCollect_ViewList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_WordCollect"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_WordCollect_ViewList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_WordCollect_ViewList");
		obj.put("idfieldname", "id");
		obj.put("sortname", "WordName");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：Word导出总表新增页面接口"
			, notes = "接口说明：Word导出总表新增页面接口。<br>"
			+ "使用位置：配置管理-Word导出总表-增加<br>"
			+ "逻辑说明：点击增加按钮新增配置时，调用该接口获取一下初始化数据")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request, Model model) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_WordCollect";
		String viewname = "add";
	...
	@ApiOperation(value = "接口说明：Word导出总表修改页面接口"
			, notes = "接口说明：Word导出总表修改页面接口。<br>"
			+ "使用位置：配置管理-Word导出总表-修改<br>"
	...
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_WordCollect_MyList";
	...
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		MySqlData mySqlData = new MySqlData();
		//数据库表名
		mySqlData.setTableName("SM_WordCollect");
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		//权限校验
		//执行删除操作
		Integer iResult = service.delete("SM_WordCollect", id);
		//删除成功后可以删除关联子表数据
		if(iResult > 0) {
			return R.ok();
		} else {
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入
		String importname = "SM_WordCollect_Import";
