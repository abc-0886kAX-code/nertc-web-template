 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-通用配置页面主表接口", tags = "系统管理-通用配置页面主表接口")
@Controller("SM_CommonPage_Controller")
@RequestMapping(value = "/SystemManage/SM_CommonPage")
public class SM_CommonPage_Controller extends BaseController {
	@Resource
	private CommonService service;

	@ApiOperation(value = "接口说明：MyList_PC通用配置页面主表录入列表接口", notes = "接口说明：MyList_PC通用配置页面主表录入列表接口")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request) throws Exception {
		Map<String, Object> obj = new HashMap<String, Object>();
		// 搜索和列表配置
		obj.put("searchSchema", service.getSearchJson("SM_CommonPage_MyList"));
		obj.put("columnSchema", service.getListJson("SM_CommonPage_MyList"));
		// 操作权限
		...
		listparam.put("listview", "SM_CommonPage_MyList");
		obj.put("listparam", listparam);
		// 列表接口参数
		obj.put("exportname", "SM_CommonPage_MyList");
		obj.put("importname", "SM_CommonPage_Import");
	...
	@ApiOperation(value = "接口说明：ViewList_PC通用配置页面主表查看列表接口", notes = "接口说明：ViewList_PC通用配置页面主表查看列表接口")
	@GetMapping("/ViewList_PC")
	@ResponseBody
	public Map<String, Object> ViewList_PC(HttpServletRequest request) throws Exception {
		Map<String, Object> obj = new HashMap<String, Object>();
		// 搜索和列表配置
		obj.put("searchSchema", service.getSearchJson("SM_CommonPage_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_CommonPage_ViewList"));
		// 操作权限
		SysUser sysuser = DataUtils.getSysUser(request);
		List<String> powerList = new ArrayList<String>();
		powerList.add("export");
		...
		// listparam为给获取数据列表接口GetList传参
		Map<String, Object> listparam = DataUtils.getRequestMap(request);
		listparam.put("listview", "SM_CommonPage_ViewList");
		obj.put("listparam", listparam);
		// 列表接口参数
		obj.put("exportname", "SM_CommonPage_MyList");
		obj.put("idfieldname", "id");
	...
	@ApiOperation(value = "接口说明：Add_PC通用配置页面主表新增页面接口", notes = "接口说明：Add_PC通用配置页面主表新增页面接口")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		// 参数整理
		String tablename = "SM_CommonPage";
		String viewname = "add";
		// 默认值
	...
	@ApiOperation(value = "接口说明：Edit_PC通用配置页面主表修改页面接口", notes = "接口说明：Edit_PC通用配置页面主表修改页面接口")
	@ApiImplicitParams({
	...
	@ApiOperation(value = "接口说明：View_PC通用配置页面主表查看页面接口", notes = "接口说明：View_PC通用配置页面主表查看页面接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识,必填", required = true, dataType = "String", paramType = "query"), })
	@GetMapping("/View_PC")
	@ResponseBody
	...
	@ApiOperation(value = "接口说明：GetList通用配置页面主表获取数据列表接口"
			, notes = "接口说明：GetList通用配置页面主表获取数据列表接口")
	@ApiImplicitParams({
	...
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		// 得到是哪个列表
		String listview = "SM_CommonPage_MyList";
	...
	/*@ApiOperation(value = "接口说明：GetListById通用配置页面主表通过id获取单条数据接口", notes = "接口说明：GetListById通用配置页面主表通过id获取单条数据接口")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识,必填", required = true, dataType = "String", paramType = "query"), })
	...
	@ApiOperation(value = "接口说明：Save通用配置页面主表保存数据接口", notes = "接口说明：Save通用配置页面主表保存数据接口")
	@ApiImplicitParams({
		@ApiImplicitParam(name = "pagename", value = "pagename", required = true, dataType = "String", paramType = "query")
	})
	@PostMapping("/Save")
	@ResponseBody
	@Transactional
	public HashMap<String, Object> 	Save(MultipartHttpServletRequest request) throws Exception {
		...
		MySqlData mySqlData = new MySqlData();
		// 数据库表名
		mySqlData.setTableName("SM_CommonPage");
		...
		if (StringUtils.isBlank(id)) {
			if (StringUtils.isBlank(request.getParameter("pageid"))) {
				...
				mySqlData.setFieldValue("pageid", pageid);
			}
			// 执行插入操作
		...
		if (result.getSuccess()) {
			...
			idMap.put("id",id);
			idMap.put("pageid",pageid);
			return R.ok().put("data", idMap);
		} else {
			return R.error(result.getMsg());
		}
	}

	@ApiOperation(value = "接口说明：Delete通用配置页面主表删除数据接口", notes = "接口说明：Delete通用配置页面主表删除数据接口。")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识,必填", required = true, dataType = "String", paramType = "query"), })
	@PostMapping("/Delete")
	@ResponseBody
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		// 删除成功后可以删除关联子表数据
		MySqlData mySqlData = new MySqlData();
		mySqlData.setSelectField("pageid", "pageid");
		mySqlData.setTableName("SM_CommonPage");
		mySqlData.setFieldWhere("id", id, "in");
		List<HashMap<String, Object>> list = service.getDataList(mySqlData);
		for (HashMap<String, Object> map : list) {
			String pageid = DataUtils.getMapKeyValue(map, "pageid");
			if (StringUtils.isNotBlank(pageid)) {
				service.delete("SM_CommonPage_Handler", pageid, "pageid");
				service.delete("SM_CommonPage_Tab", pageid, "pageid");
			}
		}
		// 权限校验
		// 执行删除操作
		Integer iResult = service.delete("SM_CommonPage", id);
		if (iResult > 0) {
			return R.ok();
		}
	...
	@ApiOperation(value = "接口说明：ExcelExport通用配置页面主表Excel导出接口"
			, notes = "接口说明：ExcelExport通用配置页面主表Excel导出接口")
	@ApiImplicitParams({
	...
	@ApiOperation(value = "接口说明：ExcelImport通用配置页面主表Excel导入接口", notes = "接口说明：ExcelImport通用配置页面主表Excel导入接口")
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		// Excel导入
		String importname = "SM_CommonPage_Import";
