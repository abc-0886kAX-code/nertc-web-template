 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-Excel导出列配置接口", tags = "系统管理-Excel导出列配置接口")
@Controller("SM_ExcelItem_Controller")
@RequestMapping(value = "/SystemManage/SM_ExcelItem")
public class SM_ExcelItem_Controller extends BaseController{
	@Resource
	private CommonService service;

	@ApiOperation(value = "接口说明：Excel导出列管理录入页面接口"
			, notes = "接口说明：Excel导出列管理录入页面接口。<br>"
			+ "使用位置：配置管理-Excel导出列管理-录入<br>"
			+ "逻辑说明：获取Excel导出列管理录入页面搜索条件和列表信息等页面元素")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		if(StringUtils.isNotBlank(request.getParameter("exportname"))) {
			obj.put("exportname", request.getParameter("exportname"));
		}
		obj.put("searchSchema", service.getSearchJson("SM_ExcelItem_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ExcelItem_MyList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_ExcelItem"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExcelItem_MyList");
	...
	@ApiOperation(value = "接口说明：Excel导出列管理查看页面接口"
			, notes = "接口说明：Excel导出列管理查看页面接口。<br>"
			+ "使用位置：配置管理-Excel导出列管理-查看<br>"
			+ "逻辑说明：获取Excel导出列管理查看页面搜索条件和列表信息等页面元素")
	...
	public Map<String, Object> ViewList_PC(HttpServletRequest request, Model model) throws Exception {
		...
		obj.put("searchSchema", service.getSearchJson("SM_ExcelItem_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_ExcelItem_ViewList"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExcelItem_ViewList");
		obj.put("listparam", listparam);
		obj.put("idfieldname", "id");
		obj.put("sortname", "visible desc,columnnumber");
		obj.put("sortorder", "ASC");
	...
	@ApiOperation(value = "接口说明：Excel导出列管理新增页面接口"
			, notes = "接口说明：Excel导出列管理新增页面接口。<br>"
			+ "使用位置：配置管理-Excel导出列管理-增加<br>"
			+ "逻辑说明：点击增加按钮新增配置时，调用该接口获取一下初始化数据")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request, Model model) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_ExcelItem";
		String viewname = "add";
	...
	@ApiOperation(value = "接口说明：Excel导出列管理修改页面接口"
			, notes = "接口说明：Excel导出列管理修改页面接口。<br>"
			+ "使用位置：配置管理-Excel导出列管理-修改<br>"
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "exportname", value = "exportname", required = true, dataType = "String",paramType="query"),
	})
	@PostMapping("/GetList")
	@ResponseBody
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_ExcelItem_MyList";
		if(request.getParameter("listview") != null) {
			listview = request.getParameter("listview");
		}
		//默认过滤条件
		if(StringUtils.isNotBlank(request.getParameter("exportname"))){
			mySqlData.setFieldWhere("exportname", request.getParameter("exportname"), "=");
		}
	...
	public HashMap<String, Object> GetListById(HttpServletRequest request) throws Exception {
		...
		HashMap<String, Object> obj = service.getMapByKey("SM_ExcelItem", id);
		return R.ok(obj);
	}
	...
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		//保存默认值
		if(StringUtils.isNotBlank(request.getParameter("exportname"))){
			mySqlData.setFieldValue("exportname", request.getParameter("exportname"));
		}
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		//权限校验
		//执行删除操作
		Integer iResult = service.delete("SM_ExcelItem", id);
		//删除成功后可以删除关联子表数据
		if(iResult > 0) {
			return R.ok();
		} else {
	...
	@ApiOperation(value = "接口说明：Excel导出列接口"
			, notes = "接口说明：Excel导出列接口。<br>"
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入
		String importname = "SM_ExcelItem_Import";
