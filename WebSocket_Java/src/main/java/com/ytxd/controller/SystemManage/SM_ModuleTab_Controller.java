...
@Controller("SM_ModuleTab_Controller")
@RequestMapping(value = "/SystemManage/SM_ModuleTab")
public class SM_ModuleTab_Controller extends BaseController {

    @Resource
    private CommonService service;

    @ApiOperation(value = "接口说明：移动端配置Tab表录入列表页面接口"
			, notes = "接口说明：移动端配置Tab表录入列表页面接口。<br>")
    @ApiImplicitParams({
	...
	public Map<String, Object> MyList_PC(HttpServletRequest request) throws Exception {
		...
		obj.put("searchSchema", service.getSearchJson("SM_ModuleTab_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ModuleTab_MyList"));
		...
		listparam.put("listview", "SM_ModuleTab_MyList");
		listparam.put("modulename", request.getParameter("modulename"));
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ModuleTab_MyList");
		obj.put("importname", "SM_ModuleTab_Import");
		obj.put("idfieldname", "id");
		obj.put("sortname", "id");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
    @ApiOperation(value = "接口说明：移动端配置Tab表查看列表页面接口"
			, notes = "接口说明：移动端配置Tab表查看列表页面接口。<br>")
    @ApiImplicitParams({
		@ApiImplicitParam(name = "modulename", value = "传主表name的值，必填。", required = true, dataType = "String", paramType = "query"),
    })
	...
	public Map<String, Object> ViewList_PC(HttpServletRequest request) throws Exception {
		Map<String, Object> obj = new HashMap<String, Object>();
		obj.put("searchSchema", service.getSearchJson("SM_ModuleTab_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_ModuleTab_ViewList"));
		SysUser sysuser = DataUtils.getSysUser(request);
		List<String> powerList = new ArrayList<String>();
		powerList.add("export");
		obj.put("powerall", powerList);
		Map<String, Object> listparam = new HashMap<String, Object>();
		listparam.put("listview", "SM_ModuleTab_ViewList");
		listparam.put("modulename", request.getParameter("modulename"));
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ModuleTab_MyList");
		obj.put("idfieldname", "id");
    ...
    @ApiOperation(value = "接口说明：移动端配置Tab表新增页面接口"
			, notes = "接口说明：移动端配置Tab表新增页面接口。")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_ModuleTab";
		String viewname = "add";
    ...
    @ApiOperation(value = "接口说明：移动端配置Tab表查看页面接口"
			, notes = "接口说明：移动端配置Tab表查看页面接口。")
    @ApiImplicitParams({
		@ApiImplicitParam(name = "id", value = "唯一标识，多个用英文逗号分隔。", required = true, dataType = "String", paramType = "query"),
    })
	@GetMapping("/View_PC")
    ...
    @ApiOperation(value = "接口说明：移动端配置Tab表录入和查看等列表得到列表数据"
			, notes = "接口说明：移动端配置Tab表录入和查看等列表得到列表数据。"
			+ "使用数据库表：SM_ModuleTab")
	@ApiImplicitParams({
    ...
    public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
        ...
        //得到是哪个列表
        String listview = "SM_ModuleTab_MyList";
    ...
    @ApiOperation(value = "接口说明：移动端配置Tab表保存数据接口"
			, notes = "接口说明：移动端配置Tab表保存数据接口。"
			+ "使用数据库表：SM_ModuleTab")
	@PostMapping("/Save")
	@ResponseBody
	@Transactional
    public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
        ...
        MySqlData mySqlData = new MySqlData();
        //数据库表名
        mySqlData.setTableName("SM_ModuleTab");
    ...
    @ApiOperation(value = "接口说明：移动端配置Tab表删除数据接口"
			, notes = "接口说明：移动端配置Tab表删除数据接口。"
			+ "使用数据库表：SM_ModuleTab")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "唯一标识，多个用英文逗号分隔。", required = true, dataType = "String", paramType = "query"),
	})
    ...
    public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
        ...
        //执行删除操作
        Integer iResult = service.delete("SM_ModuleTab", id);
        if(iResult > 0) {
            return R.ok();
        } else {
    ...
    @ApiOperation(value = "接口说明：移动端配置Tab表Excel导出接口"
			, notes = "接口说明：移动端配置Tab表Excel导出接口。<br>"
