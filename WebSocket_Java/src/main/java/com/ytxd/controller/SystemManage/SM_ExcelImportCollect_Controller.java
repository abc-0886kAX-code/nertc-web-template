...
import com.ytxd.service.SystemManage.SM_ExcelImportCollect_Service;
 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-Excel导入配置接口", tags = "系统管理-Excel导入配置接口")
@Controller("SM_ExcelImportCollect_Controller")
@RequestMapping(value = "/SystemManage/SM_ExcelImportCollect")
public class SM_ExcelImportCollect_Controller extends BaseController{
	@Resource
	private CommonService service;
	@Resource
	private SM_ExcelImportCollect_Service serviceEC;

	@ApiOperation(value = "接口说明：Excel导入管理录入页面接口"
			, notes = "接口说明：Excel导入管理录入页面接口。<br>"
			+ "使用位置：配置管理-Excel导入管理-录入<br>"
			+ "逻辑说明：获取Excel导入管理录入页面搜索条件和列表信息等页面元素")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		obj.put("searchSchema", service.getSearchJson("SM_ExcelImportCollect_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ExcelImportCollect_MyList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_ExcelImportCollect"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExcelImportCollect_MyList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ExcelImportCollect_MyList");
		obj.put("importname", "SM_ExcelImportCollect_Import");
		obj.put("idfieldname", "id");
		obj.put("sortname", "importname");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：Excel导入管理查看页面接口"
			, notes = "接口说明：Excel导入管理查看页面接口。<br>"
			+ "使用位置：配置管理-Excel导入管理-查看<br>"
			+ "逻辑说明：获取Excel导入管理查看页面搜索条件和列表信息等页面元素")
	@GetMapping("/ViewList_PC")
	@ResponseBody
	public Map<String, Object> ViewList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		obj.put("searchSchema", service.getSearchJson("SM_ExcelImportCollect_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_ExcelImportCollect_ViewList"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExcelImportCollect_ViewList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ExcelImportCollect_MyList");
		obj.put("idfieldname", "id");
		obj.put("sortname", "importname");
	...
	@ApiOperation(value = "接口说明：Excel导入管理新增页面接口"
			, notes = "接口说明：Excel导入管理新增页面接口。<br>"
			+ "使用位置：配置管理-Excel导入管理-增加<br>"
			+ "逻辑说明：点击增加按钮新增配置时，调用该接口获取一下初始化数据")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request, Model model) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_ExcelImportCollect";
		String viewname = "add";
	...
	@ApiOperation(value = "接口说明：Excel导入管理修改页面接口"
			, notes = "接口说明：Excel导入管理修改页面接口。<br>"
			+ "使用位置：配置管理-Excel导入管理-修改<br>"
			+ "逻辑说明：点击修改按钮修改配置时，调用该接口获取该配置的数据")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "id", value = "配置唯一标识,必填", required = true, dataType = "String",paramType="query"),
	})
	@GetMapping("/Edit_PC")
	@ResponseBody
	...
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_ExcelImportCollect_MyList";
	...
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		if(StringUtils.isBlank(id)) {
			...
			HashMap<String, Object> map = service.getMap("SM_ExcelImportCollect", id);
			//执行更新操作
			mySqlData.setFieldWhere("id", id, "=");
			result = service.update(request, mySqlData);
			//修改保存之后的处理事件
			if(!DataUtils.getMapKeyValue(map, "importname").equals(request.getParameter("importname"))) {
				MySqlData updateItem = new MySqlData();
				updateItem.setTableName("SM_ExcelImportItem");
				updateItem.setFieldValue("importname", request.getParameter("importname"));
				updateItem.setFieldWhere("importname", DataUtils.getMapKeyValue(map, "importname"), "=");
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入
		String importname = "SM_ExcelImportCollect_Import";
...
//		SysUser sysuser = DataUtils.getSysUser(request);
	...
	@SuppressWarnings("null")
	@ApiOperation(value = "接口说明：初始化导入模板"
			, notes = "接口说明：初始化导入模板。<br>"
			+ "使用位置：初始化导入模板<br>"
			+ "逻辑说明：通过id初始化导入模板"
			+ "使用数据库表：SM_BuiltCollect")
	@ApiImplicitParams({
	...
	@GetMapping("/ExcelImportInit")
	@ResponseBody
	public Map<String, Object> ExcelImportInit(HttpServletRequest request) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String id = request.getParameter("id");
		if(StringUtils.isNotBlank(id)) {
			id = id.replace(" ", "").replace("'", "");
