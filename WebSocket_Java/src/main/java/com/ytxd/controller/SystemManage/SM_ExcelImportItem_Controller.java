 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-Excel导入列配置接口", tags = "系统管理-Excel导入列配置接口")
@Controller("SM_ExcelImportItem_Controller")
@RequestMapping(value = "/SystemManage/SM_ExcelImportItem")
public class SM_ExcelImportItem_Controller extends BaseController{
	@Resource
	private CommonService service;

	@ApiOperation(value = "接口说明：Excel导入列管理录入页面接口"
			, notes = "接口说明：Excel导入列管理录入页面接口。<br>"
			+ "使用位置：配置管理-Excel导入列管理-录入<br>"
			+ "逻辑说明：获取Excel导入列管理录入页面搜索条件和列表信息等页面元素")
	@GetMapping("/MyList_PC")
	@ResponseBody
	public Map<String, Object> MyList_PC(HttpServletRequest request, Model model) throws Exception {
		Map<String, Object> obj = new HashMap<>();
		if(StringUtils.isNotBlank(request.getParameter("importname"))) {
			obj.put("importname", request.getParameter("importname"));
		}
		obj.put("searchSchema", service.getSearchJson("SM_ExcelImportItem_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ExcelImportItem_MyList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_ExcelImportItem"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExcelImportItem_MyList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ExcelImportItem_MyList");
		obj.put("idfieldname", "id");
		obj.put("sortname", "visible desc,columnnumber");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：Excel导入列管理查看页面接口"
			, notes = "接口说明：Excel导入列管理查看页面接口。<br>"
			+ "使用位置：配置管理-Excel导入列管理-查看<br>"
			+ "逻辑说明：获取Excel导入列管理查看页面搜索条件和列表信息等页面元素")
	...
	public Map<String, Object> ViewList_PC(HttpServletRequest request, Model model) throws Exception {
		...
		obj.put("searchSchema", service.getSearchJson("SM_ExcelImportItem_ViewList"));
		obj.put("columnSchema", service.getListJson("SM_ExcelImportItem_ViewList"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ExcelImportItem_ViewList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ExcelImportItem_MyList");
		obj.put("idfieldname", "id");
		obj.put("sortname", "visible desc,columnnumber");
	...
	@ApiOperation(value = "接口说明：Excel导入列管理新增页面接口"
			, notes = "接口说明：Excel导入列管理新增页面接口。<br>"
			+ "使用位置：配置管理-Excel导入列管理-增加<br>"
			+ "逻辑说明：点击增加按钮新增配置时，调用该接口获取一下初始化数据")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request, Model model) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_ExcelImportItem";
		String viewname = "add";
	...
	@ApiOperation(value = "接口说明：Excel导入列管理修改页面接口"
			, notes = "接口说明：Excel导入列管理修改页面接口。<br>"
			+ "使用位置：配置管理-Excel导入列管理-修改<br>"
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "importname", value = "importname", required = true, dataType = "String",paramType="query"),
	})
	@PostMapping("/GetList")
	@ResponseBody
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_ExcelImportItem_MyList";
		if(request.getParameter("listview") != null) {
			listview = request.getParameter("listview");
		}
		//默认过滤条件
		if(StringUtils.isNotBlank(request.getParameter("importname"))){
			mySqlData.setFieldWhere("importname", request.getParameter("importname"), "=");
		}
	...
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		//保存默认值
		if(StringUtils.isNotBlank(request.getParameter("importname"))){
			mySqlData.setFieldValue("importname", request.getParameter("importname"));
		}
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		//权限校验
		//执行删除操作
		Integer iResult = service.delete("SM_ExcelImportItem", id);
		//删除成功后可以删除关联子表数据
		if(iResult > 0) {
			return R.ok();
		} else {
	...
	@ApiOperation(value = "接口说明：Excel导入列接口"
			, notes = "接口说明：Excel导入列接口。<br>"
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入列
		String importname = "SM_ExcelImportItem_Import";
	...
	public HashMap<String, Object> UpdateField(HttpServletRequest request) throws Exception {
		...
		if(StringUtils.isBlank(id)) {
			...
			} else if("onlyflag".equalsIgnoreCase(field)) {
				mySqlData.setFieldValue("onlyflag", value);
