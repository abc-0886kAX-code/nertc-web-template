 ...
 *  控制层(前后台交互)
 */
@Api(value = "系统管理-流程步骤接口", tags = "系统管理-流程步骤接口")
@Controller("SM_ProcessItem_Controller")
@RequestMapping(value = "/SystemManage/SM_ProcessItem")
public class SM_ProcessItem_Controller extends BaseController{
	@Resource
	private CommonService service;

	@ApiOperation(value = "接口说明：流程步骤录入页面接口"
			, notes = "接口说明：流程步骤录入页面接口。<br>"
			+ "使用位置：配置管理-流程步骤-录入<br>"
			+ "逻辑说明：获取流程步骤录入页面搜索条件和列表信息等页面元素")
	...
	public Map<String, Object> MyList_PC(HttpServletRequest request, Model model) throws Exception {
		...
		if(StringUtils.isNotBlank(request.getParameter("processid"))) {
			obj.put("processid", request.getParameter("processid"));
		}
		if(StringUtils.isNotBlank(request.getParameter("code"))) {
			obj.put("code", request.getParameter("code"));
		}
		obj.put("searchSchema", service.getSearchJson("SM_ProcessItem_MyList"));
		obj.put("columnSchema", service.getListJson("SM_ProcessItem_MyList"));
		//总体增删改权限
		SysUser sysuser = DataUtils.getSysUser(request);
		obj.put("powerall", getPower(sysuser, "SM_ProcessItem"));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ProcessItem_MyList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ProcessItem_MyList");
		obj.put("importname", "SM_ProcessItem_Import");
		obj.put("idfieldname", "processstepid");
		obj.put("sortname", "orderid");
		obj.put("sortorder", "ASC");
		return R.ok().put("data", obj);
	}
	@ApiOperation(value = "接口说明：流程步骤查看页面接口"
			, notes = "接口说明：流程步骤查看页面接口。<br>"
			+ "使用位置：配置管理-流程步骤-查看<br>"
			+ "逻辑说明：获取流程步骤查看页面搜索条件和列表信息等页面元素")
	...
	public Map<String, Object> ViewList_PC(HttpServletRequest request, Model model) throws Exception {
		...
		obj.put("searchSchema", service.getSearchJson("SM_BuSM_ProcessItem_ViewListiltCollect_ViewList"));
		obj.put("columnSchema", service.getListJson(""));
		Map<String,Object> listparam = new HashMap<>();
		listparam.put("listview", "SM_ProcessItem_ViewList");
		obj.put("listparam", listparam);
		obj.put("exportname", "SM_ProcessItem_MyList");
		obj.put("idfieldname", "processstepid");
		obj.put("sortname", "orderid");
	...
	@ApiOperation(value = "接口说明：流程步骤新增页面接口"
			, notes = "接口说明：流程步骤新增页面接口。<br>"
			+ "使用位置：配置管理-流程步骤-增加<br>"
			+ "逻辑说明：点击增加按钮新增配置时，调用该接口获取一下初始化数据")
	@GetMapping("/Add_PC")
	@ResponseBody
	public Map<String, Object> Add_PC(HttpServletRequest request, Model model) throws Exception {
		SysUser sysuser = DataUtils.getSysUser(request);
		String tablename = "SM_ProcessItem";
		String viewname = "add";
		HashMap<String, Object> obj = new HashMap<String, Object>();
		if(StringUtils.isNotBlank(request.getParameter("processid"))){
			obj = service.getMap("SM_ProcessCollect", request.getParameter("processid"), "processid");
			obj.put("excelname", DataUtils.getMapKeyValue(obj, "listname"));
		}
	...
	@ApiOperation(value = "接口说明：流程步骤修改页面接口"
			, notes = "接口说明：流程步骤修改页面接口。<br>"
			+ "使用位置：配置管理-流程步骤-修改<br>"
	...
	public Map<String, Object> Edit_PC(HttpServletRequest request, Model model) throws Exception {
		String processstepid = request.getParameter("processstepid");
		if(StringUtils.isBlank(processstepid)) {
...
//		SysUser sysuser = DataUtils.getSysUser(request);
		...
		HashMap<String, Object> obj = service.getMapByKey(tablename, processstepid, "processstepid");
	...
	@ApiImplicitParams({
			...
			@ApiImplicitParam(name = "processid", value = "processid", required = true, dataType = "String",paramType="query"),
	})
	@PostMapping("/GetList")
	@ResponseBody
	public Map<String, Object> GetList(HttpServletRequest request) throws Exception {
		...
		//得到是哪个列表
		String listview = "SM_ProcessItem_MyList";
		if(request.getParameter("listview") != null) {
			listview = request.getParameter("listview");
		}
		//默认过滤条件
		if(StringUtils.isNotBlank(request.getParameter("processid"))){
			mySqlData.setFieldWhere("processid", request.getParameter("processid"), "=");
		}
	...
	public HashMap<String, Object> Save(MultipartHttpServletRequest request) throws Exception {
		...
		//保存默认值
		if(StringUtils.isNotBlank(request.getParameter("processid"))){
			mySqlData.setFieldValue("processid", request.getParameter("processid"));
		}
		...
		if(StringUtils.isBlank(processstepid)) {
			...
			//执行更新操作
			mySqlData.setFieldWhere("processstepid", processstepid, "=");
	...
	@ApiOperation(value = "接口说明：删除数据接口"
			...
			+ "使用位置：系统中删除记录的地方<br>"
			+ "逻辑说明：将指定processstepid的记录删除"
			+ "使用数据库表：SM_ProcessItem")
	@ApiImplicitParams({
			@ApiImplicitParam(name = "processstepid", value = "唯一标识,必填", required = true, dataType = "String",paramType="query"),
	})
	...
	public HashMap<String, Object> Delete(HttpServletRequest request) throws Exception {
		...
		//权限校验
		//执行删除操作
		Integer iResult = service.delete("SM_ProcessItem", processstepid, "processstepid");
		//删除成功后可以删除关联子表数据
		if(iResult > 0) {
			return R.ok();
		} else {
	...
	public Map<String, Object> ExcelImport(MultipartHttpServletRequest request) throws Exception {
		...
		//Excel导入
		String importname = "SM_ProcessItem_Import";
